    ...
    questions: [
      {
        ...
        options: [
          "A. Encrypt traffic in transit between a client and a server",
          "B. Translate human-friendly hostnames into IP addresses",
          "C. Distribute incoming requests across a pool of servers",
          "D. Store and serve the website's files and page content",
        ],
        answer: "B",
        explanation:
          "DNS resolves a hostname like www.example.com into the IP address a machine can route to. Encrypting traffic in transit is the job of TLS, spreading requests across a server pool is what a load balancer such as ELB does, and holding the site's files is the role of storage like S3 or a web server.",
      },
      {
        ...
        answer: "C",
        explanation:
          "The suffix .com is the top-level domain. In contrast, example.com is the second-level domain (SLD), www is a host label/subdomain beneath it, and 'example' by itself is only the SLD's label without any TLD.",
      },
      {
        ...
        answer: "A",
        explanation:
          "A resolver keeps an answer only for the length of the record's TTL (time to live), then re-queries. The domain registrar and the number of characters in the SLD have no bearing on caching, and the client's operating system simply honors the TTL rather than overriding it.",
      },
      {
        ...
        answer: "B",
        explanation:
          "Resolution begins at the Root name servers, which delegate to the TLD name servers such as .com, which in turn point to the SLD name servers that return the authoritative IP. Walking that chain backwards, or starting at the registrar or ISP, is not how DNS delegation works.",
      },
      {
        q: "A 'Domain Registrar' is best described as:",
        options: [
          "A. The server that stores your website's files",
          "B. Where you register/purchase a domain name",
          "C. The protocol that encrypts DNS queries in transit",
          "D. A specific type of DNS record, such as A or MX",
        ],
        answer: "B",
        explanation:
          "A registrar (such as Amazon Route 53 or GoDaddy) is the company where you register and purchase a domain, typically for an annual fee. It often bundles DNS hosting, but storing website files is a web server's job, encrypting DNS is a protocol concern, and an individual record type is something you create inside a zone.",
      },
    ],
  },
    ...
    questions: [
      {
        ...
        answer: "B",
        explanation:
          "An AAAA record maps a hostname to an IPv6 address. An A record maps to an IPv4 address instead, a CNAME points a hostname at another hostname, and an NS record lists the name servers that are authoritative for the zone.",
      },
      {
        q: "You need internal service names to resolve ONLY within your VPC. Which hosted zone type?",
        options: ["A. Public Hosted Zone", "B. Private Hosted Zone", "C. Reverse DNS lookup zone", "D. Top-level domain zone"],
        answer: "B",
        explanation:
          "A Private Hosted Zone answers records only inside the VPC(s) you associate with it, which is exactly the internal-only requirement. A Public Hosted Zone is Internet-facing, a reverse lookup zone maps IP addresses back to names, and a top-level domain zone is not something you create in Route 53 for this.",
      },
      {
        q: "You will change a record's target next week and want the cutover to propagate quickly. What should you do in advance?",
        options: [
          "A. Raise the TTL to 24 hours a few days ahead",
          "B. Lower the TTL (e.g. to 60s) ahead of time",
          "C. Delete and recreate the hosted zone entirely",
          "D. Change the record over to an NS type",
        ],
        answer: "B",
        explanation:
          "Lowering the TTL in advance means resolvers cache the current value only briefly, so once you change the target the new answer spreads quickly. Raising the TTL does the opposite and prolongs stale caching, while recreating the zone or switching record types does not speed propagation and risks an outage.",
      },
      {
        ...
        answer: "A",
        explanation:
          "Route 53 is the only AWS service that offers a 100% availability SLA. It also registers domains, supports both public and private hosted zones, and is an authoritative DNS service, so the remaining statements are false.",
      },
      {
        ...
        answer: "B",
        explanation:
          "A hosted zone costs roughly $0.50 per month each, on top of per-query charges. It is not free, and the larger figures overstate the actual price by 10x or 100x.",
      },
    ],
  },
    ...
    questions: [
      {
        ...
        options: ["A. CNAME (points to a hostname)", "B. Alias record (type A or AAAA)", "C. TXT record (holds text data)", "D. NS record (lists name servers)"],
        answer: "B",
        explanation:
          "A CNAME cannot exist at the zone apex (the root example.com), so pointing the root at a load balancer requires an Alias record — a type A/AAAA record that targets an AWS resource. A TXT record only holds arbitrary text and an NS record delegates the zone, so neither can map the apex to an ALB.",
      },
      {
        q: "Which is a UNIQUE advantage of an Alias record over a CNAME?",
        options: [
          "A. It can point to any external hostname, including non-AWS ones",
          "B. It works at the zone apex and is free with native health checks",
          "C. It lets you set a custom TTL value directly on the record itself",
          "D. It supports MX-style mail-exchange routing to your mail servers",
        ],
        answer: "B",
        explanation:
          "An Alias record's distinctive strengths are that it works at the zone apex, costs nothing, includes native health checks, and auto-follows an AWS resource's changing IPs. Pointing to any external hostname is really a CNAME trait, an Alias TTL is managed automatically rather than set by you, and mail delivery is handled by MX records.",
      },
      {
        q: "Which target is NOT valid for a Route 53 Alias record?",
        options: [
          "A. A CloudFront distribution endpoint",
          "B. An S3 bucket set up as a static website",
          "C. An EC2 instance's public DNS name",
          "D. An Application Load Balancer (ALB)",
        ],
        answer: "C",
        explanation:
          "An Alias record cannot target an EC2 instance's DNS name, because Alias targets must be AWS-managed endpoints. CloudFront distributions, S3 static-website endpoints, and load balancers are all valid Alias targets; to reach an EC2 instance you use an A record pointing at its IP instead.",
      },
      {
        q: "A CNAME record can be created for which of the following?",
        options: [
          "A. example.com (the zone apex root)",
          "B. www.example.com (a subdomain)",
          "C. An AWS resource, for free and health-checked",
          "D. An IPv4 address directly, with no hostname",
        ],
        answer: "B",
        explanation:
          "A CNAME is valid only for non-root names, so www.example.com works while the zone apex example.com does not. Pointing directly at an AWS resource with free health checks describes an Alias record, and mapping straight to an IPv4 address is what an A record does.",
      },
      {
        q: "What is the record type of a Route 53 Alias that points to an AWS resource, and can you set its TTL?",
        options: [
          "A. CNAME; yes, you configure the TTL value yourself",
          "B. A or AAAA; no, the TTL is set automatically",
          "C. NS; yes, you choose the TTL value",
          "D. TXT; no, the TTL cannot be changed",
        ],
        answer: "B",
        explanation:
          "An Alias record is always type A or AAAA, and Route 53 manages its TTL automatically — you cannot choose it. Answers that name CNAME, NS, or TXT, or that claim you set the TTL yourself, misdescribe how Alias records behave.",
      },
    ],
  },
    ...
    questions: [
      {
        ...
        options: ["A. Latency-based routing", "B. Weighted routing", "C. Failover routing", "D. Geolocation routing"],
        answer: "B",
        explanation:
          "Weighted routing splits traffic across records by relative weight, so a 90/10-style percentage split is exactly its purpose for canary and blue-green rollouts. Latency-based routing optimizes for speed, failover routing is for disaster recovery, and geolocation routing decides by where the user is located.",
      },
      {
        q: "Which policy routes each user to the AWS Region that will give them the fastest response times?",
        options: ["A. Geolocation-based routing", "B. Latency-based routing", "C. Simple routing", "D. Multi-Value routing"],
        answer: "B",
        explanation:
          "Latency-based routing measures network latency and sends each user to the Region that responds fastest for them. Geolocation routing selects by the user's physical location rather than measured speed, while simple and multi-value routing do not optimize performance at all.",
      },
      {
        ...
        options: [
          "A. Latency-based routing; no additional record is needed",
          "B. Geolocation; add a Default record for unmatched locations",
          "C. Weighted routing; plus a separate canary weight record",
          "D. Failover routing; plus a required health check on the primary",
        ],
        answer: "B",
        explanation:
          "Serving content by the user's country is geolocation routing's job, and you should add a Default record so users who match no rule still receive an answer. Latency-based routing keys off speed, weighted off percentages, and failover off health — none of them selects by country.",
      },
      {
        q: "In an active-passive Failover routing setup, what is REQUIRED on the primary record?",
        options: [
          "A. A configured weight of 100",
          "B. A health check",
          "C. A geoproximity bias value",
          "D. Multiple value answers",
        ],
        answer: "B",
        explanation:
          "Failover routing requires a health check on the primary record so Route 53 can tell when it is down and switch to the secondary. A fixed weight, a geoproximity bias, and multi-value answers all belong to other routing policies, not to failover.",
      },
      {
        q: "Which statement about Multi-Value routing is TRUE?",
        options: [
          "A. It returns up to 8 healthy records and is a full replacement for a load balancer",
          "B. It returns up to 8 healthy records but is NOT a substitute for a load balancer",
          "C. It always returns exactly one record to the client",
          "D. It cannot be associated with health checks at all",
        ],
        answer: "B",
        explanation:
          "Multi-value routing can return up to eight healthy records for client-side balancing, but it is not a substitute for a load balancer. Claiming it fully replaces an ELB overstates it, and it neither returns only a single record nor is barred from using health checks.",
      },
    ],
  },
    ...
    questions: [
      {
        ...
        options: [
          "A. Expose the entire VPC to the public internet",
          "B. Create a CloudWatch alarm and have a health check monitor that alarm",
          "C. Configure a direct TCP health check against the instance's private IP",
          "D. It is simply not possible to health-check private resources",
        ],
        answer: "B",
        explanation:
          "Because the health checkers live outside your VPC, you monitor a private endpoint indirectly: publish a CloudWatch metric, raise an alarm on it, and create a health check that watches that alarm. Exposing the VPC to the internet is insecure, a direct TCP probe still cannot reach a private IP, and private resources are far from impossible to monitor.",
      },
      {
        q: "Which HTTP status codes cause a Route 53 endpoint health check to PASS?",
        options: ["A. Only an exact 200", "B. 2xx and 3xx codes", "C. Any status code, even 5xx", "D. Only a 301 redirect"],
        answer: "B",
        explanation:
          "A Route 53 health check passes on any 2xx or 3xx response. Limiting success to exactly 200 or to 301 alone is too narrow, and treating every code including 5xx as healthy would defeat the check's whole point of detecting failures.",
      },
      {
        ...
        options: [
          ...
          "B. A latency-based routing policy for the checks",
          "C. Three separate hosted zones, one per child check",
          "D. A CloudWatch monitoring dashboard for them",
        ],
        answer: "A",
        explanation:
          "A calculated health check aggregates up to 256 child checks with OR/AND/NOT logic and lets you require how many must pass, which fits a 2-of-3 rule perfectly. A routing policy, extra hosted zones, and a dashboard do not combine child health-check results this way.",
      },
      {
        q: "To let Route 53 health checkers reach your endpoint behind a firewall, you must:",
        options: [
          "A. Completely disable the firewall for all inbound traffic",
          "B. Allow the published Route 53 health-checker IP ranges",
          "C. Whitelist every source with 0.0.0.0/0",
          "D. Switch to using only TCP health checks",
        ],
        answer: "B",
        explanation:
          "You must allow the published Route 53 health-checker IP ranges (from ip-ranges.json) inbound so the external probes can reach your endpoint. Disabling the firewall or opening it to every source is insecure, and changing the check protocol does not get traffic through the firewall.",
      },
      {
        q: "What is the primary purpose of Route 53 health checks?",
        options: [
          "A. To cache DNS answers at resolvers for longer",
          "B. To enable automated DNS failover to healthy resources",
          "C. To register and automatically renew domain names",
          "D. To encrypt DNS queries in transit and prevent snooping",
        ],
        answer: "B",
        explanation:
          "Route 53 health checks exist to enable automated DNS failover, so queries stop resolving to unhealthy endpoints and move to healthy ones. Caching duration is set by TTL, registering and renewing domains is a registrar function, and encrypting queries is unrelated to health checking.",
      },
    ],
  },
    ...
    questions: [
      {
        ...
        options: [
          "A. Transfer the domain to Amazon Registrar before you can manage its DNS",
          "B. Create a Route 53 hosted zone and update the registrar's NS records",
          "C. Create only a Private Hosted Zone for the domain",
          "D. Nothing at all; it switches over automatically",
        ],
        answer: "B",
        explanation:
          "Registrar and DNS service are separate roles, so you create a Route 53 hosted zone and update the NS records at your registrar to point to Route 53's name servers. A full domain transfer is not required, a private zone cannot serve public DNS, and nothing happens on its own.",
      },
      {
        ...
        answer: "B",
        explanation:
          "An Inbound Endpoint lets on-premises resolvers send queries into the VPC to resolve AWS names such as EC2 instances and Private Hosted Zone records. An Outbound Endpoint handles the opposite direction, and a NAT gateway or internet gateway provides network routing, not DNS resolution.",
      },
      {
        ...
        answer: "B",
        explanation:
          "An Outbound Endpoint forwards Route 53 Resolver queries out to your on-premises DNS resolvers, which is what EC2 instances need to resolve on-prem names. An Inbound Endpoint is the reverse direction, and a public hosted zone or Alias record does not forward queries to on-prem.",
      },
      {
        q: "Which statement best captures the registrar vs DNS-service distinction?",
        options: [
          "A. They are always run by the same company and can never be separate",
          "B. A registrar registers the domain; a DNS service manages records, and they can differ",
          "C. Only a registrar, never a DNS service, may create the records",
          "D. A DNS service must be bought before you can register a domain",
        ],
        answer: "B",
        explanation:
          "Registration and DNS management are separate roles: a registrar sells and registers the domain, while a DNS service hosts and manages its records, and the two can be different providers such as GoDaddy plus Route 53. They need not be the same company, either the registrar's or a third party's DNS can hold the records, and no DNS service is needed just to buy a domain.",
      },
      {
        q: "By default, the Route 53 Resolver automatically answers queries for all of the following EXCEPT:",
        options: [
          "A. EC2 instances' local domain names",
          "B. Records in a Private Hosted Zone",
          "C. Records from public name servers",
          "D. Your on-premises DNS zones without any endpoint",
        ],
        answer: "D",
        explanation:
          "By default the Route 53 Resolver answers queries for EC2 instances' local domain names, records in Private Hosted Zones, and records from public name servers. It does not resolve your on-premises DNS zones on its own — that requires an Outbound Endpoint to forward the queries.",
      },
    ],
  },
];
